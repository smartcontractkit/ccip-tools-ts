...
async function main() {
  await yargs(hideBin(process.argv))
    ...
    .command(
      'sendMultiple <deployments-path>',
      'interactively select multiple source and destination chains to send CCIP messages',
      (yargs) =>
        yargs
      ...
      async (argv) => {
        return sendMultiple(argv).catch((err) => {
          process.exitCode = 1
          if (!logParsedError(err)) console.error(err)
        })
      },
