...
export abstract class Chain<F extends ChainFamily = ChainFamily> {
   ...
   *
   * @throws {@link CCIPTransactionNotFinalizedError} if transaction is not finalized after timeout
   *
  ...
  /**
   * Fetch a CCIP request by message ID.
   *
   * Default implementation uses the API client.
   * Subclasses may override to fetch from chain as fallback.
   *
   * @param messageId - Message ID to fetch request for
   * @param _opts - Optional parameters (onRamp may be required in some implementations)
   * @returns Promise resolving to CCIPRequest
   *
   * @throws {@link CCIPApiClientNotAvailableError} if apiClient was disabled
   * @throws {@link CCIPMessageIdNotFoundError} if message not found (transient)
   ...
   *
   * @throws {@link CCIPMessageBatchIncompleteError} if not all messages found (transient)
   *
   ...
   * @returns Promise resolving to fee amount in fee token units
   *
   ...
   *   wallet,
   * })
   * console.log(`Executed: ${receipt.tx.hash}`)
   * ```
   */
   ...
   * @returns Promise resolving to CCIPCommit info
   *
   * @throws {@link CCIPCommitNotFoundError} if commit not found (transient)
   *
   ...
   * @returns Promise resolving to CCIP execution object
   *
   * @throws {@link CCIPExecTxRevertedError} if no receipt found in transaction
   *
   ...
   * @param token - Token address
   * @returns Promise resolving to token configuration
   *
   * @throws {@link CCIPTokenNotInRegistryError} if token not in registry
   *
   * @example Get token pool for token
   * ```typescript
   * const config = await chain.getRegistryTokenConfig(registry, tokenAddress)
   * console.log(`TokenPool: ${config.tokenPool}`)
   * ```
   */
   ...
   * @returns Promise resolving to pool configuration
   *
   * @throws {@link CCIPLegacyTokenPoolsUnsupportedError} if legacy token pool
   *
   * @example Get token pool configuration
   * ```typescript
   * const config = await chain.getTokenPoolConfigs(tokenPoolAddress)
   * console.log(`Router: ${config.router}, Token: ${config.token}`)
   * ```
   */
   ...
   *
   * @param tokenPool - Token pool address
   * @param remoteChainSelector - If provided, only return remotes for the specified chain
   * @returns Promise resolving to record of network names and remote configurations
   *
   * @throws {@link CCIPTokenPoolChainConfigNotFoundError} if remote chain not configured
   *
   * @example Get remote pool configurations
   * ```typescript
   * const remotes = await chain.getTokenPoolRemotes(tokenPoolAddress)
   * for (const [network, config] of Object.entries(remotes)) {
   *   console.log(`${network}: ${config.remoteToken}`)
   * }
   * ```
   */
