...
import type { FinalityAllowed } from '../extra-args.ts'
 ...
 *
 * Carries everything a contract-verification call needs that isn't already known from the
 * deploy result: which bundled contract was deployed and its ABI-encoded constructor args
 * (the exact bytes appended to the init code). Feed these to the SDK `verifyDeployedContract`
 * helper (in the `verify` subpath) as the encoded-args input. Present on EVM deploys only.
 */
export interface DeployVerification {
  /** Bundled verification-registry key, e.g. `'CrossChainToken'` (see `listDeployableContracts()`). */
  contract: string
...
export interface DeployTokenResult {
  ...
  /** EVM only: handle to verify the deployed token on a source-chain explorer. */
...
/**
 * EVM-specific parameters for deploying a canonical CCT v2.0 `CrossChainToken`.
 *
 * Constructor: `(ConstructorParams args, address burnMintRoleAdmin, address owner)` where
 * `ConstructorParams = (name, symbol, maxSupply, preMint, preMintRecipient, decimals, ccipAdmin)`.
 * `initialSupply` maps to `preMint`. When `preMint` is `0n` the `preMintRecipient` must be the
 * zero address (enforced automatically). All address fields default to `ownerAddress`, and
 * `ownerAddress` is required on the unsigned path / auto-filled from the signer on the signed path.
 *
 * @example
 * ```typescript
 * const params: EVMDeployTokenParams = { name: 'My Token', symbol: 'MTK', decimals: 18 }
 * ```
 */
export interface EVMDeployTokenParams extends DeployTokenParams {
  /**
   * Owner address (2-step `AccessControlDefaultAdminRules` admin). Required on the unsigned
   * path; auto-filled from the signer on the signed path. Defaults the other address fields.
   */
  ownerAddress?: string
  /** CCIP admin (`getCCIPAdmin()`), used by `registration-method get-ccip-admin`. Defaults to `ownerAddress`. */
  ccipAdmin?: string
  /** Admin that may grant/revoke MINTER/BURNER roles. Defaults to `ownerAddress`. */
  burnMintRoleAdmin?: string
  /** Recipient of the `preMint` (`initialSupply`). Defaults to `ownerAddress`; ignored when `initialSupply` is `0n`. */
  preMintRecipient?: string
}

/**
 * EVM-specific parameters for deploying a `CrossChainPoolToken` — the CCT v2.0 contract that is
 * simultaneously an ERC20 token and its own CCIP token pool (single deploy).
 *
 * Constructor: `(ConstructorParams tokenParams, address advancedPoolHooks, address rmnProxy, address router)`.
 */
export interface EVMDeployCrossChainPoolTokenParams extends DeployTokenParams {
  /** CCIP Router address (used to derive `rmnProxy` via `getArmProxy()`). */
  routerAddress: string
  /** Advanced pool hooks contract. Default: zero address (no hooks). */
  advancedPoolHooks?: string
  /** CCIP admin (`getCCIPAdmin()`). Required on the unsigned path; auto-filled from the signer on the signed path. */
  ccipAdmin?: string
  /** Recipient of the `preMint` (`initialSupply`). Defaults to `ccipAdmin`; ignored when `initialSupply` is `0n`. */
  preMintRecipient?: string
}

/**
 * Result of {@link deployCrossChainPoolToken}. The single deployed `address` is simultaneously the
 * token and its pool, so `tokenAddress` and `poolAddress` are equal to it.
 */
export interface DeployCrossChainPoolTokenResult {
  ...
  txHash: string
  /** Handle to verify the deployed combined token+pool on a source-chain explorer. */
  verification?: DeployVerification
}

// ─── EVM TokenPoolFactory v2 ────────────────────────────────────────────────────
...
/**
 * Params for {@link EVMTokenAdmin.deployTokenAndPoolViaFactory} — deploy a new CrossChainToken
 * **and** its token pool in one transaction through a `TokenPoolFactory 2.0.0` (CREATE2).
 */
export interface EVMFactoryDeployTokenAndPoolParams {
  ...
  decimals: number
  maxSupply: bigint
  /** Initial supply minted at deploy (default 0). */
  preMint?: bigint
  /** Recipient of the pre-mint (default: the future owner). */
  preMintRecipient?: string
  poolType: 'burn-mint' | 'lock-release'
  /** Existing `ERC20LockBox` for lock-release; the factory auto-deploys one when omitted. */
  lockBoxAddress?: string
  /** CREATE2 salt (a random 32-byte value is used when omitted). */
  salt?: string
  /** Final owner of the token + pool (defaults to the signer). */
  futureOwner?: string
}

/**
 * Params for {@link EVMTokenAdmin.deployPoolViaFactory} — deploy a token pool for an **existing**
 * token through a `TokenPoolFactory 2.0.0` (CREATE2).
 */
export interface EVMFactoryDeployPoolParams {
  /** The `TokenPoolFactory 2.0.0` address on this chain. */
  factoryAddress: string
  ...
  /** CREATE2 salt (a random 32-byte value is used when omitted). */
  salt?: string
  /** Final owner of the pool (defaults to the signer). */
  futureOwner?: string
}

/** Result of {@link EVMTokenAdmin.deployTokenAndPoolViaFactory}. */
export interface FactoryDeployTokenAndPoolResult {
  /** The deployed CrossChainToken address. */
  tokenAddress: string
  /** The deployed token-pool address. */
  poolAddress: string
  /** Deploy transaction hash. */
  txHash: string
  /** Lock-release only: the `ERC20LockBox` bound to the pool. */
  lockBoxAddress?: string
  /** Verification handles for every contract the factory deployed (token, pool, [lockbox]). */
  verifications: DeployVerificationTarget[]
}

/** Result of {@link EVMTokenAdmin.deployPoolViaFactory}. */
export interface FactoryDeployPoolResult {
  /** The deployed token-pool address. */
  poolAddress: string
  /** Deploy transaction hash. */
  txHash: string
  /** Lock-release only: the `ERC20LockBox` bound to the pool. */
  lockBoxAddress?: string
  /** Verification handles for every contract the factory deployed (pool, [lockbox]). */
  verifications: DeployVerificationTarget[]
}
...
export interface DeployPoolResult {
  ...
  initialized?: boolean
  /**
   * EVM lock-release only: address of the `ERC20LockBox` bound to the token. The signed
   * `deployPool` auto-deploys one and returns it here; for burn-mint pools this is omitted.
   */
  lockBoxAddress?: string
  /** EVM only: handle to verify the deployed pool on a source-chain explorer. */
  verification?: DeployVerification
  /** EVM lock-release only: handle to verify the auto-deployed `ERC20LockBox`. */
...
/**
 * EVM-specific parameters for deploying a canonical CCT v2.0 CCIP token pool.
 *
 * v2.0 constructors:
 ...
 *
 * `rmnProxy` is derived automatically via `Router.getArmProxy()`; `advancedPoolHooks` defaults to
 * the zero address. For `lock-release`, the signed `deployPool` auto-deploys an `ERC20LockBox`;
 * the unsigned `generateUnsignedDeployPool` requires `lockBoxAddress`.
 *
 * @example
 * ```typescript
...
export interface EVMDeployPoolParams extends DeployPoolParams {
  ...
  advancedPoolHooks?: string
  /**
   * `lock-release` only: address of an existing `ERC20LockBox`. Required by the unsigned path;
   * the signed `deployPool` auto-deploys one when omitted.
   */
  lockBoxAddress?: string
}
...
// ── Provide Liquidity (lock-release pools) ──────────────────────────────────

/**
 * Parameters for {@link provideLiquidity} — funds a lock-release token pool with
 * liquidity so it can release tokens on inbound CCIP transfers. **EVM lock-release
 * pools only** (burn-mint pools mint on demand and hold no liquidity).
 *
 * The operation is version-aware:
 * - **v1.5 / v1.6**: liquidity is held by the pool. `approve(pool, amount)` then
 *   `pool.provideLiquidity(amount)` (caller must be the pool's rebalancer).
 * - **v2.0**: liquidity lives in a separate `ERC20LockBox`. `approve(lockBox, amount)`
 *   then `lockBox.deposit(token, 0, amount)` (deposit is permissionless).
 *
 * @example
 * ```typescript
...
/**
 * Unified result from {@link provideLiquidity}.
 *
 * @example
 * ```typescript
 * const { txHash } = await admin.provideLiquidity(wallet, params)
 * console.log(`Liquidity provided, tx: ${txHash}`)
 * ```
 */
export interface ProvideLiquidityResult {
  /** Transaction hash of the provide/deposit transaction (the second of the two txs). */
  txHash: string
}

// ── Set Token Transfer Fee Config (EVM v2.0+ only) ──────────────────────────

/**
 * A single per-destination token-transfer fee config update.
 *
 * Pairs a destination chain selector with the full {@link TokenTransferFeeConfig}
 * (the 7 fields the v2.0 pool stores, including `isEnabled`).
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export interface TokenTransferFeeConfigUpdate {
...
/**
 * Parameters for {@link setTokenTransferFeeConfig} — sets per-destination token
 * transfer fee configs on a CCIP token pool. **EVM v2.0+ pools only.**
 *
 * Encodes `applyTokenTransferFeeConfigUpdates(args[], disable[])` in a single tx:
 * enabled/updated configs go in `updates`, while `disable` lists destination
 * selectors whose fee config should be cleared. Access: pool owner or fee admin.
 *
 * @example
 * ```typescript
 * const params: SetTokenTransferFeeConfigParams = {
 *   poolAddress: '0x1234...',
 *   updates: [{
 *     remoteChainSelector: 14767482510784806043n,
 *     config: {
 ...
 * ```
 */
export interface SetTokenTransferFeeConfigParams {
  /** Local pool address. */
  poolAddress: string
  /** Per-destination fee config updates (can be empty if only disabling). */
  updates: TokenTransferFeeConfigUpdate[]
  /** Destination chain selectors whose fee config should be disabled/cleared. */
  disable?: bigint[]
}

/**
 * Unified result from {@link setTokenTransferFeeConfig}.
 *
 * @example
 * ```typescript
 * const { txHash } = await admin.setTokenTransferFeeConfig(wallet, params)
 * console.log(`Token transfer fee config updated, tx: ${txHash}`)
 * ```
 */
export interface SetTokenTransferFeeConfigResult {
  /** Transaction hash/signature. */
  txHash: string
}

// ── Set Allowed Finality Config (EVM v2.0+ only) ────────────────────────────

/**
 * Parameters for {@link setAllowedFinalityConfig} — sets the bytes4 allowed-finality
 * config on a CCIP token pool. **EVM v2.0+ pools only.** Access: pool owner.
 *
 * `finality` is encoded to a bytes4 via the SDK finality codec:
 * - `'finalized'` → `0x00000000` (full finality)
 * - `'safe'` → safe-head flag set
 * - a number → that many block confirmations (Faster-Than-Finality), [0..65535]
 * - a {@link FinalityAllowed} object → combine the safe flag with a block depth
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export interface SetAllowedFinalityConfigParams {
  /** Local pool address. */
  poolAddress: string
...
/**
 * Unified result from {@link setAllowedFinalityConfig}.
 *
 * @example
 * ```typescript
 * const { txHash } = await admin.setAllowedFinalityConfig(wallet, params)
 * console.log(`Allowed finality config updated, tx: ${txHash}`)
 * ```
 */
export interface SetAllowedFinalityConfigResult {
  /** Transaction hash/signature. */
  txHash: string
}

// ── Set Fee Admin (EVM v2.0+ only) ──────────────────────────────────────────

/**
 * Parameters for {@link setFeeAdmin} — delegates token-transfer fee management to a
 * separate admin address on a CCIP token pool. **EVM v2.0+ pools only.**
 *
 * Reads the current dynamic config `(router, rateLimitAdmin, feeAdmin)` and rewrites
 * only the `feeAdmin`, preserving `router` and `rateLimitAdmin`. Access: pool owner.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export interface SetFeeAdminParams {
  /** Local pool address. */
  poolAddress: string
...
/**
 * Unified result from {@link setFeeAdmin}.
 *
 * @example
 * ```typescript
 * const { txHash } = await admin.setFeeAdmin(wallet, params)
 * console.log(`Fee admin updated, tx: ${txHash}`)
 * ```
 */
export interface SetFeeAdminResult {
  /** Transaction hash/signature. */
  txHash: string
}
...
/**
 * EVM result from querying mint/burn roles on a CrossChainToken token.
 *
