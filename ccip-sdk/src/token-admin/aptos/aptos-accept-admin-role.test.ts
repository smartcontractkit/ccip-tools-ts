...
// =============================================================================
// AptosTokenAdmin — acceptAdminRole
// =============================================================================

describe('AptosTokenAdmin — acceptAdminRole', () => {
  // ===========================================================================
  ...
  describe('generateUnsignedAcceptAdminRole — validation', () => {
    ...
    it('should accept valid params (fails at Aptos provider)', async () => {
      ...
      await assert.rejects(
        () => admin.generateUnsignedAcceptAdminRole(sender, validParams),
        (err: unknown) => {
          assert.ok(!(err instanceof CCIPAcceptAdminRoleParamsInvalidError))
          return true
        },
  ...
  describe('acceptAdminRole — wallet validation', () => {
    ...
    it('should reject undefined wallet', async () => {
      await assert.rejects(
        () => admin.acceptAdminRole(undefined, validParams),
        (err: unknown) => {
          assert.ok(err instanceof CCIPWalletInvalidError)
          return true
        },
