...
export class SolanaTokenAdmin extends SolanaChain {
  ...
  async generateUnsignedApplyChainUpdates(
    ...
    const selectorsBeingRemoved = new Set(params.remoteChainSelectorsToRemove)
    ...
    for (const chain of params.chainsToAdd) {
      const chainSelectorBuf = Buffer.alloc(8)
      chainSelectorBuf.writeBigUInt64LE(chain.remoteChainSelector)
  ...
  async generateUnsignedAppendRemotePoolAddresses(
    ...
    const chainSelectorBuf = Buffer.alloc(8)
    chainSelectorBuf.writeBigUInt64LE(params.remoteChainSelector)
  ...
  async generateUnsignedSetChainRateLimiterConfig(
    ...
    for (const config of params.chainConfigs) {
      const chainSelectorBuf = Buffer.alloc(8)
      chainSelectorBuf.writeBigUInt64LE(config.remoteChainSelector)
   ...
   */
  async revokeMintBurnAccess(_wallet: unknown, _params: RevokeMintBurnAccessParams): Promise<never> {
