...
export async function getUsdcAttestation(
  ...
  const res = await fetch(`${circleApiBaseUrl}/messages/${sourceDomain}?transactionHash=${txHash}`)
      ...
      if (dataLength(extraData) === 64) {
        ...
        try {
          return {
            _tag: 'usdc',
            extraData,
            ...(await getUsdcAttestation(
              { sourceDomain, nonce, txHash: request.tx.hash },
              networkType,
            )),
          }
        } catch (err) {
        ...
        try {
          return {
            _tag: 'lbtc',
            extraData,
            ...(await getLbtcAttestation(extraData, networkType)),
          }
        } catch (err) {
