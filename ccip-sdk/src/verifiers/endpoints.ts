/**
 * Parse `--verifier` / `--ccv-data` style inputs into the endpoint model the transport uses.
 *
 * These are pure string helpers (no I/O, browser-safe): the grammar is generic and shared by the
 * CLI and any other consumer, so it lives in the SDK rather than being duplicated per tool.
 *
 * @packageDocumentation
 */
import { CCIPError, CCIPErrorCode } from '../errors/index.ts'
import type { VerifierEndpoint } from './transport.ts'
 ...
 * @example
 * ```ts
 * parseVerifierEntry('0xabc…=grpc+plaintext://localhost:15051')
 * // → { ccvAddress: '0xabc…', endpoint: { type: 'aggregator', target: 'localhost:15051', tls: false, raw: 'grpc+plaintext://localhost:15051' } }
 * ```
 */
export function parseVerifierEntry(entry: string): {
  ...
  if (!trimmed) throw new CCIPError(CCIPErrorCode.ARGUMENT_INVALID, '--verifier entry is empty')
  ...
  // Strip trailing slashes with a linear reverse scan, not a backtracking regex: `/\/+$/`
  // on library input is a js/polynomial-redos (quadratic on long runs of '/').
  const afterScheme = urlPart.slice(sep + 3)
  let slashEnd = afterScheme.length
  while (slashEnd > 0 && afterScheme[slashEnd - 1] === '/') slashEnd--
  const target = afterScheme.slice(0, slashEnd)
  const known = SCHEMES[scheme]
  if (!known) {
    throw new CCIPError(
      CCIPErrorCode.ARGUMENT_INVALID,
      `--verifier entry has an unsupported scheme "${scheme}://": ${SCHEME_HELP}`,
    )
  }
  if (!target)
    throw new CCIPError(CCIPErrorCode.ARGUMENT_INVALID, `--verifier entry has no host: "${entry}"`)
...
export function parseCcvData(entries: readonly string[]): SuppliedCcvData[] {
  ...
  for (const raw of entries.flatMap((e) => e.split(',')).map((e) => e.trim())) {
    ...
    if (!/^0x[0-9a-fA-F]{40}$/.test(ccvAddress)) {
      throw new CCIPError(
        CCIPErrorCode.ARGUMENT_INVALID,
        `--ccv-data entry has an invalid CCV address "${ccvAddress}"`,
      )
    }
