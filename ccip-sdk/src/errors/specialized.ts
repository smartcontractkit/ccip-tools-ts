...
/** Thrown when provideLiquidity parameters are invalid. */
export class CCIPProvideLiquidityParamsInvalidError extends CCIPError {
  override readonly name = 'CCIPProvideLiquidityParamsInvalidError'
  /** Creates a params-invalid error for provide liquidity. */
  constructor(param: string, reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.PROVIDE_LIQUIDITY_PARAMS_INVALID,
      `Invalid provideLiquidity param "${param}": ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, param, reason },
...
/** Thrown when the provideLiquidity transaction fails. */
export class CCIPProvideLiquidityFailedError extends CCIPError {
  override readonly name = 'CCIPProvideLiquidityFailedError'
  /** Creates a provide liquidity failed error. */
  constructor(reason: string, options?: CCIPErrorOptions) {
    super(CCIPErrorCode.PROVIDE_LIQUIDITY_FAILED, `Provide liquidity failed: ${reason}`, {
      ...options,
      isTransient: false,
      context: { ...options?.context, reason },
    })
...
/** Thrown when setTokenTransferFeeConfig parameters are invalid. */
export class CCIPSetTokenTransferFeeConfigParamsInvalidError extends CCIPError {
  override readonly name = 'CCIPSetTokenTransferFeeConfigParamsInvalidError'
  /** Creates a params-invalid error for set token transfer fee config. */
  constructor(param: string, reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.SET_TOKEN_TRANSFER_FEE_CONFIG_PARAMS_INVALID,
      `Invalid setTokenTransferFeeConfig param "${param}": ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, param, reason },
...
/** Thrown when the setTokenTransferFeeConfig transaction fails. */
export class CCIPSetTokenTransferFeeConfigFailedError extends CCIPError {
  override readonly name = 'CCIPSetTokenTransferFeeConfigFailedError'
  /** Creates a set token transfer fee config failed error. */
  constructor(reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.SET_TOKEN_TRANSFER_FEE_CONFIG_FAILED,
      `Set token transfer fee config failed: ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, reason },
...
/** Thrown when setAllowedFinalityConfig parameters are invalid. */
export class CCIPSetAllowedFinalityConfigParamsInvalidError extends CCIPError {
  override readonly name = 'CCIPSetAllowedFinalityConfigParamsInvalidError'
  /** Creates a params-invalid error for set allowed finality config. */
  constructor(param: string, reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.SET_ALLOWED_FINALITY_CONFIG_PARAMS_INVALID,
      `Invalid setAllowedFinalityConfig param "${param}": ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, param, reason },
...
/** Thrown when the setAllowedFinalityConfig transaction fails. */
export class CCIPSetAllowedFinalityConfigFailedError extends CCIPError {
  override readonly name = 'CCIPSetAllowedFinalityConfigFailedError'
  /** Creates a set allowed finality config failed error. */
  constructor(reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.SET_ALLOWED_FINALITY_CONFIG_FAILED,
      `Set allowed finality config failed: ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, reason },
...
/** Thrown when setFeeAdmin parameters are invalid. */
export class CCIPSetFeeAdminParamsInvalidError extends CCIPError {
  override readonly name = 'CCIPSetFeeAdminParamsInvalidError'
  /** Creates a params-invalid error for set fee admin. */
  constructor(param: string, reason: string, options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.SET_FEE_ADMIN_PARAMS_INVALID,
      `Invalid setFeeAdmin param "${param}": ${reason}`,
      {
        ...options,
        isTransient: false,
        context: { ...options?.context, param, reason },
...
/** Thrown when the setFeeAdmin transaction fails. */
export class CCIPSetFeeAdminFailedError extends CCIPError {
  override readonly name = 'CCIPSetFeeAdminFailedError'
  /** Creates a set fee admin failed error. */
  constructor(reason: string, options?: CCIPErrorOptions) {
    super(CCIPErrorCode.SET_FEE_ADMIN_FAILED, `Set fee admin failed: ${reason}`, {
      ...options,
      isTransient: false,
      context: { ...options?.context, reason },
    })
...
/**
 * Thrown when a contract-verification request to an explorer errors (bad request,
 * HTTP failure, unknown solc version, unreadable source, etc.).
 *
 * The explorer's raw `result` body, when present, is attached on `context.result`.
 * The message is preserved verbatim so callers can match explorer markers
 * (e.g. "Contract source code already verified").
 *
 * @example
 * ```typescript
 * try {
 ...
 * } catch (error) {
 *   if (error instanceof CCIPContractVerificationError) {
 *     console.log(`Explorer error: ${error.message}`, error.context.result)
 *   }
 * }
 * ```
 */
export class CCIPContractVerificationError extends CCIPError {
  override readonly name = 'CCIPContractVerificationError'
  /** Creates a contract verification error. */
  constructor(message: string, options?: CCIPErrorOptions) {
    super(CCIPErrorCode.CONTRACT_VERIFICATION_ERROR, message, {
      ...options,
...
/**
 * Thrown when a contract cannot be verified (no known verifier for the chain,
 * recompile mismatch, etc.).
 *
 * @example
 * ```typescript
 * try {
 *   await verifyDeployedContract(params)
 * } catch (error) {
 *   if (error instanceof CCIPContractVerificationFailedError) {
 *     console.log(`Verification failed: ${error.context.reason}`)
 *   }
 * }
 * ```
 */
export class CCIPContractVerificationFailedError extends CCIPError {
  override readonly name = 'CCIPContractVerificationFailedError'
  /** Creates a contract verification failed error. */
  constructor(reason: string, options?: CCIPErrorOptions) {
    super(CCIPErrorCode.CONTRACT_VERIFICATION_FAILED, `Contract verification failed: ${reason}`, {
      ...options,
      isTransient: false,
      context: { ...options?.context, reason },
    })
...
/**
 * Thrown when an unknown contract name is requested from the bundled verification registry.
 *
 * @example
 * ```typescript
 * try {
 *   getVerificationArtifact('NotABundledContract')
 * } catch (error) {
 *   if (error instanceof CCIPUnknownVerificationContractError) {
 *     console.log(`Available: ${error.context.available}`)
 *   }
 * }
 * ```
 */
export class CCIPUnknownVerificationContractError extends CCIPError {
  override readonly name = 'CCIPUnknownVerificationContractError'
  /** Creates an unknown verification contract error. */
  constructor(contract: string, available: readonly string[], options?: CCIPErrorOptions) {
    super(
      CCIPErrorCode.VERIFICATION_CONTRACT_UNKNOWN,
      `Unknown contract "${contract}". Bundled: ${available.join(', ')}`,
      { ...options, isTransient: false, context: { ...options?.context, contract, available } },
    )
  }
}
