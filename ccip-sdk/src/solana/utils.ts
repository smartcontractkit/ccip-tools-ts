...
type ParsedLog = Pick<
  SolanaLog,
  "topics" | "index" | "address" | "data" | "level" | "type"
>;
...
function resolveTransactionAccountKeys(
...
export function parseSolanaLogs(
  ...
  const results: OrderedParsedLog[] = [];
  const invokeLogs: ({ used?: boolean } & Pick<
    ParsedLog,
    "address" | "index" | "level"
  >)[] = [];
  ...
  for (const [i, log] of logs.entries()) {
    ...
    const matchReturn =
      !matchInvoke && log.match(/^Program (\w+) (success|failed)\b/);
    const matchLog =
      !matchInvoke && !matchReturn && log.match(/^Program (log|data): /);
    if (matchInvoke) {
...
export function getErrorFromLogs(
  logs_:
    ...
    | readonly Pick<
        SolanaLog,
        "address" | "index" | "data" | "topics" | "tx" | "level" | "type"
      >[]
    | null,
  ...
  const lastProgramLogs = logs
    .reduceRight(
      (acc, l) =>
        ...
        !acc.length || (l.address === acc[0]!.address && !l.topics.length)
          ? [l, ...acc]
          : acc,
      [] as typeof logs,
    )
  ...
  if (!!logs[0] && "tx" in logs[0] && !!logs[0].tx?.error)
    Object.assign(
      ...
      Object.fromEntries(
        Object.entries(
          logs[0].tx.error as Record<string, [number, string]>,
        ).map(([k, [i, e]]) => [`${k}[${i}]`, e] as const),
      ),
    );
  return res;
}
  ...
  if (!("tx" in rest)) {
    ...
    try {
      ...
      tx = new VersionedTransaction(
        message.compileToV0Message(rest.addressLookupTableAccounts),
      );
...
function isComputeBudgetError(error: unknown): boolean {
  ...
  if (typeof structured === "string")
    return structured === "ComputationalBudgetExceeded";
  if (
    structured &&
    typeof structured === "object" &&
    "InstructionError" in structured
  ) {
    const detail = (structured as { InstructionError?: unknown })
      .InstructionError;
  ...
  try {
    for (
      ...
      do {
        ...
        includesMain =
          mainIndex != null && start <= mainIndex && mainIndex < end;
      ...
      try {
        ...
        txV0 = new VersionedTransaction(
          txMsg.compileToV0Message(addressLookupTableAccounts),
        );
    ...
    throw new CCIPPartialTransactionSubmissionError(
      committedHashes,
      committedInstructionCount,
      {
  ...
  const out: RateLimiterState = {
    ...
    get tokens() {
      const cur =
        tokens +
        this.rate *
          BigInt(Math.floor(Date.now() / 1000) - input.lastUpdated.toNumber());
