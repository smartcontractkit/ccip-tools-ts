...
function generate(opts = {}) {
  return new ProposeAdminRole().generate(stubChain(), {
    tokenAddress: TOKEN,
...
describe('Solana TokenAdminRegistry proposeAdminRole', () => {
  it('matches a direct ownerProposeAdministrator build (instruction parity)', async () => {
    ...
    const expected = await createRouterProgram(stubChain(), router, new PublicKey(PAYER))
      .methods.ownerProposeAdministrator(new PublicKey(ADMINISTRATOR))
      .accountsStrict({
  ...
  it('rejects an invalid public key before building', async () => {
    await assert.rejects(
      () => generate({ administrator: 'not-a-pubkey' }),
