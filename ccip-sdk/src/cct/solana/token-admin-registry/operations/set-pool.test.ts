...
describe('SetPool (cct/solana)', () => {
  describe('generate', () => {
    ...
    it('uses caller-provided writable indexes', async () => {
      ...
      assert.equal(
        unsigned.instructions[0]!.data.toString('hex'),
        '771e0eb473e1a7ee0400000003040709',
      )
    })
  ...
  describe('validation', () => {
    it('rejects invalid writable indexes before resolving the router', async () => {
      let routerLookups = 0
