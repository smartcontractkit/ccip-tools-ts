...
function generate(opts = {}) {
  return new AcceptAdminRole().generate(stubChain(), {
    tokenAddress: TOKEN,
...
describe('Solana TokenAdminRegistry acceptAdminRole', () => {
  it('matches a direct acceptAdminRoleTokenAdminRegistry build (instruction parity)', async () => {
  ...
  it('rejects an invalid public key before building', async () => {
    await assert.rejects(
      () => generate({ routerAddress: 'not-a-pubkey' }),
