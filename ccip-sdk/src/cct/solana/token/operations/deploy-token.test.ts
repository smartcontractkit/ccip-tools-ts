...
describe('Solana token deployToken', () => {
  ...
  it('appends idempotent ATA + mintTo instructions when initialSupply is set', async () => {
    const unsigned = await generate({ initialSupply: 1_000n })
    const [, , createAtaIx, mintToIx] = unsigned.instructions

    assert.equal(unsigned.instructions.length, 4)
    assert.ok(createAtaIx)
    assert.ok(mintToIx)
    // Associated Token Program owns the idempotent create-ATA instruction.
    assert.equal(createAtaIx.programId.toBase58(), 'ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL')
    assert.equal(mintToIx.programId.toBase58(), TOKEN_PROGRAM_ID.toBase58())
    assert.equal(mintToIx.data[0], 7) // MintTo
    // The created mint address (surfaced from execute via resultFromGenerated) is present.
    assert.match(unsigned.tokenAddress, /^[1-9A-HJ-NP-Za-km-z]+$/)
  })

  it('does not mint when initialSupply is zero or omitted', async () => {
    const unsigned = await generate({ initialSupply: 0n })
    assert.equal(unsigned.instructions.length, 2)
  })
