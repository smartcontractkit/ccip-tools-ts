...
type BaseDeployTokenParams = {
  ...
  /**
   * Optional initial supply to mint (base units) after creating the mint. When set and greater
   * than `0n`, the recipient's associated token account is created idempotently and the supply
   * is minted to it with the payer as mint authority. Omit or set to `0n` to create a mint with
   * no supply.
   */
  initialSupply?: bigint
  /**
   * Optional base58 recipient of the initial supply. Only used when `initialSupply` is set and
   * greater than `0n`. Defaults to the payer.
   */
  recipient?: string
}
...
/** Result of executing Solana token deploy, including the created mint address. */
export type ExecuteDeployTokenResult = TransactionHash & {
  /** The created mint address (base58). */
  tokenAddress: string
}
...
> {
  ...
  protected validate(params: GenerateDeployTokenParams): void {
    ...
    // POC parity: initial-supply mint params (not in DAPP-10498 deploy-token)
    if (params.initialSupply !== undefined) {
      if (typeof params.initialSupply !== 'bigint' || params.initialSupply < 0n) {
        throw new CCTParamsInvalidError(
          this.name,
          'initialSupply',
          'must be a non-negative bigint when provided',
        )
      }
      if (params.recipient !== undefined) {
        validatePublicKey(this.name, 'recipient', params.recipient)
      }
    }
  ...
  protected async buildUnsigned(
    ...
    // POC parity: mint initial supply (not in DAPP-10498 deploy-token). Mint authority is the
    // payer, matching the mint-init above. See legacy token-admin/solana/index.ts L697-736.
    const initialSupply = params.initialSupply ?? 0n
    if (initialSupply > 0n) {
      const recipient = params.recipient ? new PublicKey(params.recipient) : payer
      const ata = getAssociatedTokenAddressSync(mint, recipient, false, tokenProgram)
      instructions.push(
        createAssociatedTokenAccountIdempotentInstruction(
          payer,
          ata,
        ...
        createMintToInstruction(mint, ata, payer, initialSupply, [], tokenProgram),
      )
    }
  ...
  // POC parity: surface the created mint address from execute (not in DAPP-10498 deploy-token).
  /** Adds the created mint address to the execute result. */
  protected override resultFromGenerated(
    hash: TransactionHash,
    tx: GenerateDeployTokenResult,
  ): ExecuteDeployTokenResult {
