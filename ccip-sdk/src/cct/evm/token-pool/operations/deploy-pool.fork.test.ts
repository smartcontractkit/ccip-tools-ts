...
describe('EVMTokenManager Pool Fork Tests', { skip, timeout: 120_000 }, () => {
  ...
  it('should deploy LockReleaseTokenPool, auto-deploy a LockBox and verify contract state', async () => {
    const result = await mgr.deployPool({
    ...
    assert.ok(result.hash, 'should return tx hash')
    // signed deployPool auto-deploys an ERC20LockBox and returns its address
    assert.ok(result.lockBoxAddress, 'should return auto-deployed lockBox address')
    assert.match(result.lockBoxAddress, /^0x[0-9a-fA-F]{40}$/, 'lockBox should be valid address')
  ...
  it('should require lockBoxAddress on the unsigned lock-release path', async () => {
    await assert.rejects(
      () =>
        mgr.generateUnsignedDeployPool({
          poolType: 'lock-release',
          tokenAddress,
          localTokenDecimals: 18,
          routerAddress: SEPOLIA_ROUTER,
      ...
      (err: unknown) => {
        ...
        assert.equal(err.context.param, 'lockBoxAddress')
        return true
      },
    )
  })
  ...
  it('should produce unsigned tx that deploys successfully when signed manually', async () => {
    const unsigned = await mgr.generateUnsignedDeployPool({
      poolType: 'burn-mint',
      tokenAddress,
      localTokenDecimals: 18,
      routerAddress: SEPOLIA_ROUTER,
  ...
  // ===========================================================================
  // deployPool — with advancedPoolHooks (v2.0)
  // ===========================================================================

  it('should deploy burn-mint pool with a custom advancedPoolHooks address', async () => {
    const advancedPoolHooks = '0x0000000000000000000000000000000000000001'
    ...
    // Pool still binds to the configured token/router regardless of the hooks address.
    const pool = new Contract(result.poolAddress, POOL_ABI, provider)
    const token: string = await pool.getFunction('getToken')()
    assert.equal(token.toLowerCase(), tokenAddress.toLowerCase())
  })
