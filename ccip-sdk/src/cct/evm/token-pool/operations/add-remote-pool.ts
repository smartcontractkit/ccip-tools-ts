/**
 * addRemotePool: authorizes one more remote pool address on a lane (v1.5.1+).
 *
 * @remarks **v1.5.1 and newer.** v1.5.1 turned a lane's single remote pool into a set — several
 * remote pools may be authorized at once, which is how a remote-side pool upgrade is rolled out
 * without downtime (add the new pool, drain the old, then `removeRemotePool`). v1.5.0 has no
 * additive primitive at all, only the wholesale {@link SetRemotePool}, so this op reports itself
 * unsupported there rather than emulating an add as a replace.
 *
...
type Encoder = (iface: Interface, params: ParsedAddRemotePoolParams) => UnsignedEVMTx

const encodeAddRemotePool: Encoder = (
  iface,
...
export class AddRemotePool extends EVMOperation<AddRemotePoolParams, ParsedAddRemotePoolParams> {
  ...
  private readonly encoders: Partial<Record<TokenPoolVersion, Encoder | null>> = {
    [TokenPoolVersion.V1_5_1]: encodeAddRemotePool,
  }
  ...
  /**
   * Resolves the pool's version (rejecting v1.5.0), confirms `sender` owns the pool, then rejects
   * a duplicate: the lane's currently registered remote pools are read scoped to this one
   * selector, and an address already among them would revert on-chain
   * (`PoolAlreadyAdded`). A lane that is not configured yet reads as having none — see
   * {@link readRegisteredRemotePools}.
   ...
   * `remotePoolAddress` is already registered on this lane
   */
  protected async buildUnsigned(
    chain: EVMChain,
    params: ParsedAddRemotePoolParams,
  ...
  override async execute(
    chain: EVMChain,
    params: EVMExecuteParams<AddRemotePoolParams>,
