...
describe('EVM cct grantMintBurnAccess', () => {
  const op = new GrantMintBurnAccess()

  it('defaults to grantMintAndBurnRoles — byte-identical to a direct ethers encode', async () => {
    const unsigned = await op.generate(stubChain, { tokenAddress: TOKEN, authority: AUTHORITY })
    const expected = canonical.encodeFunctionData('grantMintAndBurnRoles', [AUTHORITY])
    assert.equal(unsigned.family, ChainFamily.EVM)
    assert.equal(unsigned.transactions[0]!.to, TOKEN)
    assert.equal(unsigned.transactions[0]!.data, expected)
  })

  it('encodes grantRole(MINTER_ROLE, authority) for role=mint', async () => {
    const unsigned = await op.generate(stubChain, {
      tokenAddress: TOKEN,
      authority: AUTHORITY,
      role: 'mint',
    })
    const expected = canonical.encodeFunctionData('grantRole', [MINTER_ROLE, AUTHORITY])
    assert.equal(unsigned.transactions[0]!.data, expected)
  })

  it('encodes grantRole(BURNER_ROLE, authority) for role=burn', async () => {
    const unsigned = await op.generate(stubChain, {
      tokenAddress: TOKEN,
      authority: AUTHORITY,
      role: 'burn',
    })
    const expected = canonical.encodeFunctionData('grantRole', [BURNER_ROLE, AUTHORITY])
    assert.equal(unsigned.transactions[0]!.data, expected)
  })
  ...
  it('rejects invalid tokenAddress before RPC', async () => {
    await assert.rejects(
      () => op.generate(stubChain, { tokenAddress: 'nope', authority: AUTHORITY }),
  ...
  it('rejects invalid authority before RPC', async () => {
    await assert.rejects(
      () => op.generate(stubChain, { tokenAddress: TOKEN, authority: 'nope' }),
