...
/** EVMChain stub answering `isBurner(address)` with `holds`. */
function stubChain({
  holds = true,
  callError,
  seen = newSeen(),
...
const op = new IsBurner()

describe('IsBurner (cct/evm)', () => {
      ...
      it(`rejects ${param} = ${value} before any RPC`, async () => {
        ...
        await assert.rejects(
          ...
          (err: unknown) =>
            ...
            err.context.operation === 'isBurner' &&
  ...
  it('rejects a contract that does not declare isBurner(address)', async () => {
    // a v2.0.0 CrossChainToken gates burning through AccessControl instead
