...
import { type RevokeBurnRoleParams, RevokeBurnRole } from './revoke-burn-role.ts'
...
const op = new RevokeBurnRole()

function generate(chain: EVMChain, overrides: Partial<RevokeBurnRoleParams> = {}) {
  return op.generate(chain, { tokenAddress: TOKEN, burner: ACCOUNT, sender: OWNER, ...overrides })
}

describe('RevokeBurnRole (cct/evm)', () => {
  describe('generate', () => {
    it('encodes revokeBurnRole(address) to the token', async () => {
      const seen = newSeen()
      ...
      it(`rejects ${param} = ${String(value)} before any RPC`, async () => {
        ...
        await assert.rejects(
          ...
          (err: unknown) =>
            ...
            err.context.operation === 'revokeBurnRole' &&
  ...
  describe('no-op guard', () => {
    it('rejects a revoke when the account does not hold the burn role', async () => {
