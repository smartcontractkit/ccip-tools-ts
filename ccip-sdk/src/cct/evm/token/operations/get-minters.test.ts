...
import { GetMinters } from './get-minters.ts'
...
const FRESH = new Interface(['function getMinters() view returns (address[])'])

/** The `eth_call`s the op makes, in order, as decoded function names. */
type Seen = { calls: string[] }
const newSeen = (): Seen => ({ calls: [] })

/** EVMChain stub answering `getMinters()` with `holders`. */
function stubChain({
...
const op = new GetMinters()

describe('GetMinters (cct/evm)', () => {
  it('lists the holders in one call', async () => {
    ...
    assert.deepEqual(seen.calls, ['getMinters'])
  })
    ...
    it(`rejects tokenAddress = ${value} before any RPC`, async () => {
      ...
      await assert.rejects(
        ...
        (err: unknown) =>
          ...
          err.context.operation === 'getMinters' &&
  ...
  it('rejects a contract that does not declare getMinters()', async () => {
