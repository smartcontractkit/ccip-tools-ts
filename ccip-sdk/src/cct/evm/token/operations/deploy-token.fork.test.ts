...
describe('EVMTokenManager Fork Tests', { skip, timeout: 60_000 }, () => {
  ...
  it('should deploy CrossChainToken and verify all contract state', async () => {
  ...
  it('should produce unsigned tx that deploys successfully when signed manually', async () => {
    const unsigned = await mgr.generateUnsignedDeployToken({
      name: 'Manual Token',
      symbol: 'MAN',
      decimals: 8,
      initialSupply: 500n,
    ...
    const token = new Contract(receipt.contractAddress, CrossChainTokenABI, provider)
    const name: string = await token.getFunction('name')()
