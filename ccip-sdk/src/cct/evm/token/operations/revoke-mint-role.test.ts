...
const expectedData = (minter = ACCOUNT) => FRESH.encodeFunctionData('revokeMintRole', [minter])
 ...
 * role state of the account being revokeed — true is the state that makes this call a real change.
 */
function stubChain({
  holdsRole = true,
  owner = OWNER,
  ...
  return {
    ...
    provider: {
      call: ({ data }: { data: string }) => {
        ...
        assert.ok(role === MINTER_ROLE || role === BURN_MINT_ADMIN_ROLE)
...
const op = new RevokeMintRole()

function generate(chain: EVMChain, overrides: Partial<RevokeMintRoleParams> = {}) {
  return op.generate(chain, { tokenAddress: TOKEN, minter: ACCOUNT, sender: OWNER, ...overrides })
}

describe('RevokeMintRole (cct/evm)', () => {
  describe('generate', () => {
    it('encodes revokeMintRole(address) to the token', async () => {
      const seen = newSeen()
    ...
    it('encodes revokeRole(MINTER_ROLE, address) for a CrossChainToken', async () => {
      ...
      assert.equal(
        ...
        V2.encodeFunctionData('revokeRole', [MINTER_ROLE, ACCOUNT]),
      )
    })
      ...
      it(`rejects ${param} = ${String(value)} before any RPC`, async () => {
        ...
        await assert.rejects(
          ...
          (err: unknown) =>
            ...
            err.context.operation === 'revokeMintRole' &&
  ...
  describe('no-op guard', () => {
    ...
    it('rejects the no-op with no sender supplied too', async () => {
      await assert.rejects(
        () => generate(stubChain({ holdsRole: false }), { sender: undefined }),
  ...
  describe('execute', () => {
    ...
    it('rejects a non-signer wallet', async () => {
      await assert.rejects(
        () => op.execute(stubChain(), { tokenAddress: TOKEN, minter: ACCOUNT, wallet: {} }),
