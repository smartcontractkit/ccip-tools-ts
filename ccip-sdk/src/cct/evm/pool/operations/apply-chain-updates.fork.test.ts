...
describe('EVMTokenManager applyChainUpdates Fork Tests', { skip, timeout: 120_000 }, () => {
  ...
  it('should apply chain updates and verify on-chain', async () => {
    ...
    const result = await mgr.applyChainUpdates({
      poolAddress,
  ...
  it('should produce unsigned tx with correct shape', async () => {
    const unsigned = await mgr.generateUnsignedApplyChainUpdates({
      poolAddress,
      remoteChainSelectorsToRemove: [],
      chainsToAdd: [
        {
  ...
  it('should remove a remote pool address and verify on-chain', async () => {
    ...
    const result = await mgr.removeRemotePoolAddresses({
      poolAddress,
      remoteChainSelector: REMOTE_CHAIN_SELECTOR,
  ...
  it('should delete a chain config and verify on-chain', async () => {
    ...
    const result = await mgr.deleteChainConfig({
      poolAddress,
      remoteChainSelector: REMOTE_CHAIN_SELECTOR,
      wallet,
    })
