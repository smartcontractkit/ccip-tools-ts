...
describe('EVM cct appendRemotePoolAddresses', () => {
  const op = new AppendRemotePoolAddresses()

  it('encodes addRemotePool — byte-identical to a direct ethers encode (v1.6)', async () => {
    const unsigned = await op.generate(stubChain(), {
      poolAddress: POOL,
      remoteChainSelector: SELECTOR,
      remotePoolAddresses: [REMOTE_EVM],
    })
    const expected = new Interface(TokenPool_1_6_ABI).encodeFunctionData('addRemotePool', [
      SELECTOR,
  ...
  it('uses the v2.0 interface for v2.0 pools (identical selector, byte-parity)', async () => {
    ...
    const expected = new Interface(TokenPool_2_0_ABI).encodeFunctionData('addRemotePool', [
      SELECTOR,
      encodeRemote(REMOTE_EVM),
    ])
    assert.equal(unsigned.transactions[0]!.data, expected)
  ...
  it('rejects v1.5 pools (no addRemotePool)', async () => {
    await assert.rejects(
      () =>
