/**
 * removeRemotePoolAddresses — removes specific remote pool addresses from an
 * already-configured remote chain on a local TokenPool, by encoding
 * `removeRemotePool(uint64 remoteChainSelector, bytes remotePoolAddress)` — one
 * transaction per address. Unlike deleting the chain config, this preserves the
 * config and only removes the listed addresses. Requires a v1.5.1+ pool (v1.5 has
 * no `removeRemotePool`; use `applyChainUpdates` to re-initialize instead).
 *
...
/** Parameters for `removeRemotePoolAddresses`. */
export type RemoveRemotePoolAddressesParams = {
  /** Local TokenPool address (EVM). */
  poolAddress: string
  /** Remote chain selector; must already be configured via `applyChainUpdates`. */
  remoteChainSelector: bigint
  /** Remote pool addresses to remove, in native format (hex/base58/base64). At least one required. */
...
/** Removes specific remote pool addresses from an existing chain config on a local TokenPool. */
export class RemoveRemotePoolAddresses extends EVMOperation<RemoveRemotePoolAddressesParams> {
  readonly name = 'removeRemotePoolAddresses'

  /** Validates the pool address, selector, and address list before any RPC. */
  protected validate(p: RemoveRemotePoolAddressesParams): void {
  ...
  /** Builds one `removeRemotePool` tx per address; rejects v1.5 pools. */
  protected async buildUnsigned(
    chain: EVMChain,
    p: RemoveRemotePoolAddressesParams,
    ...
    // removeRemotePool(uint64,bytes) encoding is version-stable across v1.5.1–v2.0.
    ...
    const transactions = p.remotePoolAddresses.map((remotePoolAddress) => ({
      to: p.poolAddress,
      data: iface.encodeFunctionData('removeRemotePool', [
