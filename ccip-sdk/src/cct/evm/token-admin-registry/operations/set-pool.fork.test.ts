...
describe('EVMTokenManager setPool Fork Tests', { skip, timeout: 120_000 }, () => {
  ...
  it('should set pool and verify on-chain', async () => {
    const result = await mgr.setPool({
      tokenAddress,
      poolAddress,
      address: SEPOLIA_ROUTER,
      wallet,
  ...
  it('should grant mint/burn access to pool and verify on-chain', async () => {
    const result = await mgr.grantMintBurnAccess({
      tokenAddress,
  ...
  it('should produce unsigned grantMintBurnAccess tx with correct shape', async () => {
    const unsigned = await mgr.generateUnsignedGrantMintBurnAccess({
      tokenAddress,
      authority: poolAddress,
    })
