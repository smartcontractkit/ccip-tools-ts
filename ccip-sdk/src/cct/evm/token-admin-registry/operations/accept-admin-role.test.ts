...
describe('EVM cct acceptAdminRole', () => {
  const op = new AcceptAdminRole()

  it('encodes acceptAdminRole against the resolved TAR — byte-identical to a direct ethers encode', async () => {
    const unsigned = await op.generate(stubChain, {
      tokenAddress: TOKEN,
      address: ROUTER,
    })
    const expected = new Interface(TokenAdminRegistryABI).encodeFunctionData('acceptAdminRole', [
      TOKEN,
    ])
  ...
  it('rejects invalid addresses before RPC', async () => {
    await assert.rejects(
      () => op.generate(stubChain, { tokenAddress: 'nope', address: ROUTER }),
