...
export class TransferAdmin extends CantonOperation<TransferAdminParams, ParsedTransferAdminParams> {
  ...
  protected async buildCommands(
    ...
    // current admin (`p.sender`). The gateway `ledgerApi` proxy authenticates
    // as the caller's Bearer token, which has CanReadAs(sender) but NOT
    // CanReadAs(ccipOwner) — so the ACS query MUST key on `sender` (the user
    // is a TokenConfig observer via admin), never ccipOwner.
