/**
 * Apply a remote-chain update to an EVM TokenPool — the "reverse link" for a
 * Canton pool: tells the EVM pool about the Canton chain, the Canton pool's
 * instance address, and the Canton's encoded instrument ID.
 *
 * Run:
 *   EVM_PRIVATE_KEY=0x… CONFIG_JSON=evm-pool.json node --experimental-strip-types ccip-sdk/examples/evm-apply-chain-updates.ts
 *
 ...
 *   "instrumentId": "TESTLINK4",                  // Canton instrument id
 *   "cantonPoolOwner": "u_…::1220…",              // Canton poolOwner party
 *   "poolInstanceId": "testlink4-pool-001",       // optional, default <instrument lowercased>-pool-001
 ...
 * calldata instead of sending (dry run). If the selector is already wired,
 * it's removed and re-added in the same tx (the pool rejects re-adding).
 *
...
async function main(): Promise<void> {
  ...
  console.error('  remotePoolAddress   ', remotePoolAddress, ` (keccak of "${poolInstanceId}@<owner>")`)
  console.error('  remoteTokenAddress  ', remoteTokenAddress, ` (keccak of "${instrumentId}@<owner>")`)
  console.error('  rate limits         ', `enabled=${enabled} capacity=${rateLimit.capacity} rate=${rateLimit.rate}`)
