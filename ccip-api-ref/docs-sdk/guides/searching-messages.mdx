...
| `q` | `string` | Free-text match on source and destination token address, source pool address, transaction hashes, messageId, sender, and receiver. Comma-separate several terms; all must match |
...
:::note
When paginating with a cursor, filters may be repeated or omitted, but must not contradict the cursor. The API answers `400 INVALID_PARAMETER_COMBINATION` if they differ.
:::
