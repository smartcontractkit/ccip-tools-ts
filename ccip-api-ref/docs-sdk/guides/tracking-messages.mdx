...
Use `CCIPAPIClient.searchAllMessages` to find all messages from a specific sender via the CCIP API:
...
for await (const msg of api.searchAllMessages({ sender: '0xSenderAddress...' })) {
  console.log('Message ID:', msg.messageId)
  console.log('Status:', msg.status)
}
```

`searchAllMessages` is an async generator that handles cursor-based pagination automatically. You can also filter by `receiver`, `sourceChainSelector`, `destChainSelector`, and `readyForManualExecOnly`. To get full message details for a specific result, use `api.getMessageById(msg.messageId)`.
...
| `api.searchAllMessages({ sender })` | CCIP API | Yes | `MessageSearchResult` (async generator with pagination) |
