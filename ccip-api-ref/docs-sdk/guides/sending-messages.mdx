...
const tx = await source.sendMessage(router, destSelector, message, { value: fee })
console.log('Sent in tx:', tx.hash)
```
...
const tx = await source.sendMessage(router, destSelector, message)
```
...
const { tx, fee } = await source.generateUnsignedSendMessage(
  router,
  destSelector,
  message
)
...
// const hash = await source.provider.sendTransaction(signedTx)
```
...
async function sendCrossChainMessage() {
  const source = await EVMChain.fromUrl(
  ...
  const requests = await source.fetchRequestsInTx(tx.hash)
  console.log('Message ID:', requests[0].message.header.messageId)
