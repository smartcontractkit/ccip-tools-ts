...
const message = {
  ...
  data: '0x',
  extraArgs: features.FINALITY_FAST != null
...
The SDK does **not** validate your `extraArgs` against the lane's on-chain version. If you pass V3 fields (e.g., `finality`) to a pre-v2.0 lane, the transaction will **revert** on-chain. Call `getLaneFeatures` first to check what the lane supports:
...
| Any of: `finality`, `ccvs`, `ccvArgs`, `executor`, `executorArgs`, `tokenReceiver`, `tokenArgs` | GenericExtraArgsV3 | `0xa69dd4aa` |
...
| `finality` | `number \| 'safe' \| 'finalized'` | `0` | Source-chain confirmations before relaying (FTF), or 'safe'/'finalized' for mode-based finality |
...
// V3 — auto-detected from finality (or any V3-only field)
const encoded = encodeExtraArgs({
  gasLimit: 200_000n,
  finality: 5,
  ccvs: [],
...
async function sendTokens(
  ...
  if (minConfirmations == null) {
    ...
    extraArgs = { gasLimit: 0n, finality: minConfirmations }
  } else {
...
| Any V3-only field present | V3 defaults: `finality: 0`, empty arrays/strings for other fields |

## Non-EVM Destinations

For Solana, Sui, and Canton destinations, `extraArgs` requires chain-specific fields. The SDK auto-populates defaults for token-only transfers, but programmable transfers need explicit configuration.
...
| **Canton** | `finality` (destination); `feeTokenHoldingCids`, `ccvRawAddresses` (source) | [Canton getting started](https://docs.chain.link/ccip/getting-started/canton) |
