...
On EVM, both simulate the `ccipReceive` call on the destination chain and return the estimated gas units, with the base transaction cost (21,000 gas) subtracted — it represents the execution cost only, suitable for use as `gasLimit` in `extraArgs` or `execute`. On Solana the same methods estimate the compute units of the OffRamp's `execute` instead (see the note below).

:::note
`estimateReceiveExecution` is implemented on **EVM** (returns `ccipReceive` gas units) and **Solana** (returns compute units — Solana meters execution in CU, not gas). Aptos, Sui, TON, and Canton do not implement it.
:::

:::tip
When called before sending with both `source` and `dest`, `estimateReceiveExecution` also runs a point-in-time destination preflight (lane gates, rate limits, LockRelease liquidity, and a pool-direct `releaseOrMint` simulation) and throws a typed error on a definitive block; checks that cannot run (destination RPC failure, or attestation-consuming pools pre-send) report themselves unavailable instead — see [Validating a Transfer Before Sending](./pre-send-validation.mdx) for exactly what is and is not covered. The returned gas number is unchanged.
:::
...
- Runs only when `estimateReceiveExecution` is available on the dest chain (EVM and Solana)
