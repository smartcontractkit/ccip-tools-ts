...
:::tip
When called before sending with both `source` and `dest`, `estimateReceiveExecution` also validates that the message can execute on the destination and throws a typed error if it cannot. For a message that carries token transfers this destination check is mandatory, not opt-in — see [Validating a Transfer Before Sending](./pre-send-validation.mdx). The returned gas number is unchanged.
:::
