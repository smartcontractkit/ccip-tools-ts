...
On v2.0+ pools, `getTokenPoolConfig` also returns `finalityDepth` (`undefined` on pre-v2.0 pools, `0` when FTF is not enabled, `>0` when enabled) and `finalitySafe` (`true` when FCR/safe finality is supported). See [Faster-Than-Finality](/sdk/guides/ftf) for the full FTF workflow.
...
TokenPool v2.0+ contracts may include separate rate limiters for Faster-Than-Finality transfers (`fastOutboundRateLimiterState` and `fastInboundRateLimiterState`). See [Faster-Than-Finality — FTF Rate Limits](/sdk/guides/ftf#ftf-rate-limits) for details and examples.
