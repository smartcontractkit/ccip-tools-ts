...
| `CCIPChainFamilyUnsupportedError` | `CHAIN_FAMILY_UNSUPPORTED` | Chain family not implemented | Use supported chain family (EVM, Solana, Aptos, Sui, TON, Canton) |
...
| `CCIPLaneLatencyInsufficientDataError` | `LANE_LATENCY_INSUFFICIENT_DATA` | Too little history to estimate lane latency | Retry without `sourceTokenAddress` for the lane-wide estimate |
...
| `CCIPTokenDataParseError` | `TOKEN_DATA_PARSE_FAILED` | Unable to parse token data | Verify token address is valid and contract is deployed |
...
| `CCIPUsdcBurnFeesError` | `USDC_BURN_FEES_FAILED` | No | Failed to fetch USDC burn fees from Circle CCTP API | Retry later or check Circle API status |
| `CCIPCctpDecodeError` | `CCTP_DECODE_FAILED` | No | Failed to decode CCTP MessageSent event from transaction | Verify this is a USDC transfer transaction |
...
| `CCIPLbtcAttestationNotApprovedError` | `LBTC_ATTESTATION_NOT_APPROVED` | Yes | LBTC attestation pending approval | Wait and retry (retryAfterMs: 30000) |
...
| `CCIPWalletChainMismatchError` | `WALLET_CHAIN_MISMATCH` | Wallet connected to a different chain than the `Chain` instance | Connect the wallet to the same chain, or build with `generateUnsigned*` and sign on the intended chain |
...
| Error | Code | When Thrown | Recovery |
|-------|------|-------------|----------|
| `CCIPVersionUnsupportedError` | `VERSION_UNSUPPORTED` | CCIP version not supported | Use supported version (v1.2, v1.5, v1.6, v2.0) |
...
| `CCIPAbortError` | `ABORT` | No | Request cancelled via `AbortSignal` | Intentional cancellation — no retry needed |
...
| `CCIPUnexpectedPaginationError` | `API_UNEXPECTED_PAGINATION` | No | Transaction contains >100 CCIP messages (unusually large) | Verify the transaction is correct |
...
| Error | Code | When Thrown | Recovery |
|-------|------|-------------|----------|
| `CCIPDataFormatUnsupportedError` | `DATA_FORMAT_UNSUPPORTED` | Unsupported data format | Use hex, bytes, or base64 |
| `CCIPDataParseError` | `DATA_PARSE_FAILED` | Could not parse the provided data | Verify data format |
| `CCIPBorshTypeUnknownError` | `BORSH_TYPE_UNKNOWN` | Unknown Borsh type in schema | Check Borsh schema definition |
| `CCIPBorshMethodUnknownError` | `BORSH_METHOD_UNKNOWN` | Unknown Borsh method | Check Borsh method name |
