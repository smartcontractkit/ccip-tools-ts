...
console.log('Chain selector:', ethereum.network.chainSelector)

// Create wallet for signing (passed to sendMessage, not fromUrl)
const wallet = new Wallet(process.env.PRIVATE_KEY!, ethereum.provider)
```
...
console.log('Connected to:', solana.network.name)
...
const message = {
  ...
  extraArgs: encodeExtraArgs({
    ...
    tokenReceiver: solanaReceiver,              // Token receiver on Solana
...
```typescript
import { SolanaChain, encodeExtraArgs, networkInfo, ChainFamily } from '@chainlink/ccip-sdk'
...
const message = {
  ...
  tokenAmounts: [],
  // Solana uses different encoding format (uint128 vs uint256)
  extraArgs: encodeExtraArgs({
    _tag: 'EVMExtraArgsV2',
    gasLimit: 200000n,
