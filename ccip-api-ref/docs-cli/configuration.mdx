...
| `CCIP_API`     | CCIP API endpoint URL, or `false` to disable API calls (decentralized mode) | `https://api.ccip.chain.link` |
...
# Or use a named keystore (pass --wallet keystore:<name> --keystore-type foundry|hardhat)
...
| ------ | --------------------------------------------------------------------------------------------------------------------------- |
| EVM    | Hex private key, path to encrypted JSON keystore, or `keystore:<name>` (use `--keystore-type` to select Foundry or Hardhat) |
...
If you manage keys with [Foundry Cast](https://book.getfoundry.sh/reference/cast/cast-wallet-import), use `keystore:<name>` with `--keystore-type foundry`:
...
If you manage keys with [Hardhat's built-in keystore](https://hardhat.org/hardhat-runner/docs/guides/configuration-variables), use `keystore:<name>` with `--keystore-type hardhat`. The CLI runs `node_modules/.bin/hardhat keystore get` directly, so Hardhat must be installed as a dev dependency in your project:
