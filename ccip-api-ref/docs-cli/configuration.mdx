...
| `OWNER_KEY`         | Alias for `USER_KEY`                                                         |
| `USER_KEY_PASSWORD` | Password for encrypted JSON keystore (file or Foundry)                       |
| `KEYSTORE_PASSWORD` | Password for Foundry Cast keystore (takes priority over `USER_KEY_PASSWORD`) |
...
If `--wallet` is omitted, the CLI automatically looks for `PRIVATE_KEY=`, `USER_KEY=`, or `OWNER_KEY=` in your `--rpcs-file` (default: `./.env`):
...
| ------ | ------------------------------------------------------------------------------------------------ |
| EVM    | Hex private key, path to encrypted JSON keystore, or `keystore:<name>` for Foundry Cast keystore |
...
If you manage keys with [Foundry Cast](https://book.getfoundry.sh/reference/cast/cast-wallet-import), use `keystore:<name>`:
