...
**Command line:** Use `-r` or `--rpcs` to specify endpoints directly:

```bash
ccip-cli show 0x123... \
  -r https://eth-sepolia.example.com \
  -r https://arb-sepolia.example.com
```
...
```bash
ccip-cli show 0x123... \
  -r "https://eth-sepolia.example.com,https://arb-sepolia.example.com"
```
...
| ------------- | ----- | -------- | -------- | ------------------------------------------------- |
| `--rpcs`      | `-r`  | string[] | -        | RPC endpoint URLs (alias: `--rpc`)                |
...
| `--page`      | -     | number   | `10000`  | Pagination size for `getLogs` queries             |
| `--no-api`    | -     | boolean  | `false`  | Disable CCIP API integration (full decentralized) |
