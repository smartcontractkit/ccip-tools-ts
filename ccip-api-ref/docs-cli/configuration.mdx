...
Instead of a static `jwt`, you can provide an `auth` object to obtain a JWT automatically via [OpenID Connect (OIDC)](https://openid.net/connect/). Three flows are supported (mirroring the Go `commonconfig.AuthConfig`):
