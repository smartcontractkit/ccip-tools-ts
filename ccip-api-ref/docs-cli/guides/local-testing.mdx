...
| `forkOf`        | no       | -             | Register a fork instead of a new chain, by id/selector/name of the forked chain; also accepted as `fork_of`. The fork inherits its selector, so `chainSelector` is not given |
...
`*` Required for a new chain; omit it when `forkOf` is set.
