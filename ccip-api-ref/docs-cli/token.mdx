...
The `token` command queries the balance of native tokens (ETH, SOL, APT, etc.) or ERC20/SPL tokens for a specified wallet address. It supports EVM, Solana, Aptos, and TON chains. Canton balance queries are not yet implemented.
...
| TON          | GRAM                   | `ton-mainnet`, `ton-testnet`                    |
| Canton       | —                      | Not supported (balance queries not implemented) |

## See Also

- [get-supported-tokens](/cli/supported-tokens) - List supported CCIP tokens
