...
### Queue Execution (Coming Soon)

:::warning
Queue execution (`--sender-queue` and `--exec-failed`) is not yet implemented. These options are accepted by the CLI but have no effect. This feature will be available in a future release.
:::
