...
3. If the RPC is a **fork** served under a different chain id (Tenderly Virtual Environment,
   `anvil --fork --chain-id`), name the forked chain instead, so the fork keeps its selector.
   Otherwise the RPC is skipped, and the error's `help:` line names its chain id and the value to
   pass:
   ```bash
