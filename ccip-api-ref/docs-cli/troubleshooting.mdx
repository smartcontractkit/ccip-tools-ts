...
```bash
ccip-cli show 0x... -r https://ethereum-sepolia-rpc.publicnode.com
```
...
2. Ensure RPC is configured for source chain:
   ```bash
   ccip-cli show 0x... -r https://source-chain-rpc.com
   ```
...
3. Verify destination RPC is configured:
   ```bash
   ccip-cli show 0x... \
     -r https://source-chain-rpc.com \
     -r https://dest-chain-rpc.com
   ```
...
3. Check rate limiter:
   ```bash
   ccip-cli getSupportedTokens source router token
   ```
...
```bash
ccip-cli getSupportedTokens source router
```
