...
TONChain.getWallet = async function loadTONWallet({
    ...
    if (typeof wallet !== 'string')
        throw new Error(`Invalid wallet option: ${wallet}`)
    ...
    if (wallet.startsWith('0x')) {
        ...
        if (secretKey.length === 32) {
            throw new Error('Invalid private key: 32-byte seeds not supported. Use 64-byte secret key or mnemonic.')
        }
    ...
    if (existsSync(wallet)) {
        ...
        const secretKey = Buffer.from(
            content.startsWith('0x') ? content.slice(2) : content,
            'hex'
        )
        if (secretKey.length !== 64) {
