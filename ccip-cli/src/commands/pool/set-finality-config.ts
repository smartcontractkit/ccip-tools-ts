...
/** Calls setAllowedFinalityConfig on the appropriate chain-family facade (EVM v2.0+ only), normalizing to `{ hash }`. */
function setForChain(
  ...
  switch (chain.network.family) {
    case ChainFamily.EVM: {
      ...
      return mgr.setAllowedFinalityConfig({ ...params, wallet })
    }
    default:
...
async function doSetFinalityConfig(ctx: Ctx, argv: SetFinalityConfigArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Allowed finality config updated, tx:', result.hash)
      return
    case Format.pretty:
    default:
