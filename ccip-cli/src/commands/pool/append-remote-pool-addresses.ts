...
/** Calls appendRemotePoolAddresses on the appropriate chain-family facade, normalizing to `{ hash }`. */
async function appendForChain(
  chain: Chain,
  wallet: unknown,
  ...
  switch (chain.network.family) {
    ...
    case ChainFamily.Aptos: {
      ...
      const { hash } = await mgr.appendRemotePoolAddresses({ ...params, wallet })
      return { hash }
    }
    default:
...
async function doAppendRemotePoolAddresses(ctx: Ctx, argv: AppendArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Remote pool addresses appended, tx:', result.hash)
      return
    case Format.pretty:
    default:
