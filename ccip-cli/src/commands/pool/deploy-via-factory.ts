...
async function doDeploy(ctx: Ctx, argv: FactoryArgv) {
  ...
  const mgr = EVMTokenManager.fromChain(chain)
  ...
  // Verification handles for every contract the factory created (pool, token, and the
  // auto-deployed lockbox for lock-release); each already carries its address.
  let verifications: VerifyTarget[]

  if (argv.tokenAddress) {
    // Existing-token mode → pool only.
    const result = await mgr.deployPoolViaFactory({
      ...shared,
    ...
    const result = await mgr.deployTokenAndPoolViaFactory({
      ...shared,
      name: argv.name,
      symbol: argv.symbol,
  ...
  // The factory-deploy facade returns a verification handle (contract key + encoded
  // constructor args + address) for every contract it created in an internal CREATE2 call.
  if (argv.verify) {
