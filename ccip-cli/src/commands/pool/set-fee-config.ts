...
function configToParams(poolAddress: string, config: ConfigFile): SetTokenTransferFeeConfigParams {
  const updates: SetTokenTransferFeeConfigParams['updates'] = config.feeConfigs.map((c) => ({
...
/** Calls setTokenTransferFeeConfig on the appropriate chain-family facade (EVM v2.0+ only), normalizing to `{ hash }`. */
function setForChain(
  chain: Chain,
  wallet: unknown,
  ...
  switch (chain.network.family) {
    case ChainFamily.EVM: {
      ...
      return mgr.setTokenTransferFeeConfig({ ...params, wallet })
    }
    default:
...
async function doSetFeeConfig(ctx: Ctx, argv: SetFeeConfigArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Token transfer fee config updated, tx:', result.hash)
      return
    case Format.pretty:
    default:
