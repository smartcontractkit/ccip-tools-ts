...
import { type SetFeeAdminParams, EVMTokenManager } from '@chainlink/ccip-sdk/src/cct/evm/index.ts'
import {
  type Chain,
  type EVMChain,
...
/** Calls setFeeAdmin on the appropriate chain-family facade (EVM v2.0+ only), normalizing to `{ hash }`. */
function setForChain(
  chain: Chain,
  wallet: unknown,
  params: SetFeeAdminParams,
): Promise<{ hash: string }> {
  switch (chain.network.family) {
    case ChainFamily.EVM: {
      const evmChain = chain as EVMChain
      const mgr = EVMTokenManager.fromChain(evmChain)
      return mgr.setFeeAdmin({ ...params, wallet })
    }
    default:
...
async function doSetFeeAdmin(ctx: Ctx, argv: SetFeeAdminArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Fee admin updated, tx:', result.hash)
      return
    case Format.pretty:
    default:
