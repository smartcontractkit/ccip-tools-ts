...
async function doDeployCombined(
  ...
  const result = await mgr.deployCrossChainPoolToken({
    name: argv.name,
    symbol: argv.symbol,
  ...
  switch (argv.format) {
    ...
    case Format.log:
      ...
      ctx.output.write('tx:', result.hash)
      break
    case Format.pretty:
    default:
  ...
  // The cct facade returns the verification handle (contract key + encoded constructor args)
  // for the deployed CrossChainPoolToken.
  if (argv.verify) {
