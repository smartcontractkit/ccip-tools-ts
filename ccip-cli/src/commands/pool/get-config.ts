...
async function doGetConfig(
  ...
  switch (argv.format) {
    case Format.json: {
      ...
      ctx.output.write(JSON.stringify(output, bigIntReplacer, 2))
      return
    }
    case Format.log:
      ctx.output.write('Pool:', poolAddress)
      ctx.output.write('Token:', poolConfig.token, tokenInfo)
      ctx.output.write('Owner:', poolConfig.owner)
      if (poolConfig.proposedOwner) ctx.output.write('Proposed Owner:', poolConfig.proposedOwner)
      if (poolConfig.rateLimitAdmin)
        ctx.output.write('Rate Limit Admin:', poolConfig.rateLimitAdmin)
      if (poolConfig.feeAdmin) ctx.output.write('Fee Admin:', poolConfig.feeAdmin)
      ctx.output.write('Router:', poolConfig.router)
      ctx.output.write('Type:', poolConfig.typeAndVersion)
      ctx.output.write('Remotes:', remotesEntries.length)
      for (const [name, remote] of remotesEntries) {
        ctx.output.write(`  ${name}:`, remote)
      }
      return
    case Format.pretty:
    default: {
        ...
        prettyTable.call(ctx, {
          ...
          ...('fastOutboundRateLimiterState' in remote && {
            ['[ftf: Faster-Than-Finality]outbound']: prettyRateLimiter(
              remote.fastOutboundRateLimiterState,
              tokenInfo,
            ),
            ['[ftf: Faster-Than-Finality]inbound']: prettyRateLimiter(
              remote.fastInboundRateLimiterState,
              tokenInfo,
            ),
          }),
        })
