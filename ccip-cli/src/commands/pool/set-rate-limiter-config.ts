...
/** Calls setChainRateLimiterConfig on the appropriate chain-family facade, normalizing to `{ hash }`. */
async function setForChain(
  ...
  switch (chain.network.family) {
    ...
    case ChainFamily.Aptos: {
      ...
      const { hash } = await mgr.setChainRateLimiterConfig({ ...params, wallet })
      return { hash }
    }
    default:
...
async function doSetRateLimiterConfig(ctx: Ctx, argv: SetRateLimiterArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Rate limiter config updated, tx:', result.hash)
      return
    case Format.pretty:
    default:
