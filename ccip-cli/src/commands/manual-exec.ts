...
export async function handler(argv: Awaited<ReturnType<typeof builder>['argv']> & GlobalOpts) {
  ...
  // Inject the Node gRPC transport for the SDK's direct verifier fetch. The SDK owns the schema and
  // the failover/dedup assembly; this transport (grpc-js) only moves bytes and is injected via the
  // existing ctx-override mechanism, so every chain built below carries it.
  ctx.verifierTransport = grpcVerifierTransport()
  // argv.senderQueue
