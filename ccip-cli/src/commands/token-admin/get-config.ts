...
async function doGetConfig(ctx: Ctx, argv: GetConfigArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('administrator:', result.administrator)
      if (result.pendingAdministrator)
        ctx.output.write('pendingAdministrator:', result.pendingAdministrator)
      if (result.tokenPool) ctx.output.write('tokenPool:', result.tokenPool)
      if (output.poolLookupTable) ctx.output.write('poolLookupTable:', output.poolLookupTable)
      if (result.poolLookupTableEntries) {
        ctx.output.write('poolLookupTableEntries:')
        result.poolLookupTableEntries.forEach((entry, i) => ctx.output.write(`  [${i}]: ${entry}`))
      }
      return
    case Format.pretty:
    default:
