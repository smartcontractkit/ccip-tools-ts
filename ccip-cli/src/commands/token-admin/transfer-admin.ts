...
/** Transfers admin using the appropriate chain-family facade, normalizing to `{ hash }`. */
async function transferAdminForChain(
  chain: Chain,
  wallet: unknown,
  argv: TransferArgv,
): Promise<{ hash: string }> {
  switch (chain.network.family) {
    ...
    case ChainFamily.Aptos: {
      ...
      const { hash } = await mgr.transferAdminRole({
        tokenAddress: argv.tokenAddress,
        newAdmin: argv.newAdmin,
        routerAddress: argv.routerAddress,
        wallet,
...
async function doTransferAdmin(ctx: Ctx, argv: TransferArgv) {
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Admin transferred, tx:', result.hash)
      return
    case Format.pretty:
    default:
