...
/** Normalized deploy outcome across chain families. `tokenAddress` is absent on Solana. */
type DeployOutcome = {
  hash: string
  tokenAddress?: string
  codeObjectAddress?: string
  metadataAddress?: string
  /** EVM-only block-explorer verification handle for `--verify`. */
  verification?: DeployVerification
}

/** Deploys via the appropriate chain-family facade, normalizing to a common outcome. */
  ...
  switch (chain.network.family) {
    ...
    case ChainFamily.Solana: {
      ...
      const result = await mgr.deployToken({
        ...
        name: argv.name,
        symbol: argv.symbol,
        ...(argv.metadataUri && { uri: argv.metadataUri }),
        ...(initialSupply !== undefined && { initialSupply }),
        wallet,
      })
      return { hash: result.hash, tokenAddress: result.tokenAddress }
    }
    case ChainFamily.Aptos: {
      ...
      return {
        hash: result.hash,
        tokenAddress: result.tokenAddress,
        ...(result.codeObjectAddress && { codeObjectAddress: result.codeObjectAddress }),
      }
    }
    default:
...
async function doDeployToken(ctx: Ctx, argv: DeployArgv) {
  ...
  if (result.tokenAddress) output.tokenAddress = result.tokenAddress
  ...
  switch (argv.format) {
    ...
    case Format.log:
      if (result.tokenAddress) {
        ctx.output.write('Token deployed:', result.tokenAddress, 'tx:', result.hash)
      } else {
        ctx.output.write('Token deployed, tx:', result.hash)
      }
  ...
  // Contract verification is EVM-only; the cct facade returns the verification handle
  // (contract key + encoded constructor args) on the deploy result.
  if (argv.verify && result.verification && result.tokenAddress) {
