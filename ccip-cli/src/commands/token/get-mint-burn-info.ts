...
async function handleEVM(
  ...
  const result = await mgr.getMintBurnRoles(tokenAddress)
  ...
  switch (argv.format) {
    case Format.json:
      ...
      return
    case Format.log:
      ctx.output.write('Token:', tokenAddress)
      ctx.output.write('Minters:', result.minters.length ? result.minters.join(', ') : '(none)')
      ctx.output.write('Burners:', result.burners.length ? result.burners.join(', ') : '(none)')
      return
    case Format.pretty:
    default:
...
async function handleSolana(
  ...
  const mgr = SolanaTokenManager.fromChain(chain)
  const result = await mgr.getMintBurnRoles(tokenAddress)
  ...
  switch (argv.format) {
    ...
    case Format.log:
      ctx.output.write('Token:', tokenAddress)
      ctx.output.write('Mint Authority:', result.mintAuthority ?? '(disabled)')
      if (result.isMultisig) {
        ctx.output.write(
          'Multisig:',
        ...
        for (const member of result.multisigMembers!) {
          ctx.output.write('  -', member.address)
        }
      }
      return
    case Format.pretty:
...
async function handleAptos(
  ...
  const mgr = AptosTokenManager.fromChain(chain)
  const result = await mgr.getMintBurnRoles(tokenAddress)
  ...
  switch (argv.format) {
    ...
    case Format.log:
      ctx.output.write('Token:', tokenAddress)
      ctx.output.write('Module:', result.tokenModule)
      if (result.owner) ctx.output.write('Owner:', result.owner)
