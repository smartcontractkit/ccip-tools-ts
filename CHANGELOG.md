...
- Solana: support reading and sending Version-1 transactions (Agave 4.x devnet and later), up to 4096 bytes per message
  - v1 messages always carry compute-unit and loaded-accounts data-size limits (SIMD-0385 caps checked locally; an RPC rejecting the v1 envelope surfaces as `CCIPTransactionTooLargeError`, so oversized `execute` still falls back to buffering). Wallets whose `supportedTransactionVersions` lack `1` (e.g. `@solana/wallet-adapter`) never get v1 — an oversized v0 message is split into smaller v0 ones or rejected; the CLI's Ledger wallet declares legacy/v0 only, since Ledger's Solana app can't parse v1 yet ([LedgerHQ/app-solana#248](https://github.com/LedgerHQ/app-solana/issues/248))
- Solana: `simulateAndSendTxs` supports `partial` (default), `resource`, and `atomic` split modes (types `SolanaSplitMode`/`SolanaSentSlice` exported); `sendMessage` no longer commits token approvals when `ccipSend` fails simulation (TOB-CLCCT-1) — a failing slice is cut right before the failed instruction, and a submission failing after earlier transactions confirmed throws `CCIPPartialTransactionSubmissionError` with `context.committedSlices` and `pendingSignature`
- SDK: v2.0 `getVerifications` merges partial answers from the CCIP API, indexers, and verifiers, resolving once the destination's CCV policy is covered; new `verifiers` (endpoint URLs tried in order) and `ccvData` (attestations obtained out of band) options, also on `execute({ messageId })`; `CCIPAPIClient.getEncodedMessage` returns a v2.0 message's encoded form and OffRamp before its verifications complete
- CLI: `manual-exec` and `show` take `--verifier <url>` to read CCV attestations straight from a verifier's aggregator (`https://` for grpc-web, `grpc://`/`grpcs://`/`grpc+plaintext://` for native gRPC); `manual-exec --ccv-data <ccv>=<0x-hex>` supplies an out-of-band attestation. A set not covering the policy fails locally, naming the missing CCVs, instead of reverting onchain
- Aptos and Sui: detect failed executions in log streams and receipts; bundled Sui fixes — deep-history `getLogs` walk ascending checkpoint slices instead of paging from the tip, empty `getOwnedObjects` lookups are memoized instead of retried for ~30s, and `offRamp` receipt filters no longer drop successful Aptos/Sui receipts
- Canton: Ledger signing support, and OAuth 2.0/OIDC authentication for the Canton Ledger API
