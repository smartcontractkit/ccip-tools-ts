...
- Solana: supports reading and sending Version-1 transactions (Agave 4.x devnet and later), up to 4096 bytes per message
  - Fix: v1 transactions always carry compute-unit and loaded-accounts data-size limits (SIMD-0385 budgets 0 for an unset one, instead of a default) — every v1 simulation failed on devnet and mainnet with `MaxLoadedAccountsDataSizeExceeded`, so v1 was never used and oversized `execute` reports failed instead of falling back to buffering: simulated at the per-tx maximums, then sent with the simulated usage plus 10%; the SIMD-0385 caps (64 accounts, 64 instructions, 12 signatures) are checked locally, and an RPC rejecting the v1 envelope surfaces as `CCIPTransactionTooLargeError`, so oversized executions still fall back to buffering
  - Wallets declaring `supportedTransactionVersions` without `1` (as in `@solana/wallet-adapter`) never get v1 transactions: one too large for v0 is split into smaller v0 ones, or rejected as too large (so `execute` still falls back to buffering). The CLI's Ledger wallet declares only legacy/v0, since Ledger's Solana app can't parse v1 yet ([LedgerHQ/app-solana#248](https://github.com/LedgerHQ/app-solana/issues/248))
...
- Solana: `sendMessage` no longer commits token approvals when `ccipSend` fails simulation — its instructions split across transactions only on transaction-size or compute-budget failures (TOB-CLCCT-1); a submission failing after earlier transactions confirmed throws `CCIPPartialTransactionSubmissionError` with `context.committedSlices` (signature and instruction range of each) and `pendingSignature` (sent but unconfirmed)
  - `simulateAndSendTxs` takes `{ computeUnits, split }` options (`split`: `'partial'` default, `'resource'`, `'atomic'`) and returns `{ hash, slices }` (types `SolanaSplitMode` / `SolanaSentSlice` exported); it cuts a failing slice right before the failed instruction (one simulation per split point, instead of one per dropped instruction), applies `computeUnits` only to the main instruction's transaction, and returns the last signature when `mainIndex` is unset (lookup-table cleanup no longer re-sends a close)
